...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    if (node == nullptr) {
      ...
      Node * copiedLeft = copy_nodes_impl(node->left);
      Node * copiedRight = copy_nodes_impl(node->right);
      return new Node(node->datum, copiedLeft, copiedRight);
    }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if(less(val,node->datum)) {
        Node * possible = min_greater_than_impl(node->left, val,less);
        if(possible!=nullptr) {
          return possible;
        }
        else {
