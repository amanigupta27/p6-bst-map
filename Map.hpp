...
public:
  ...
  Value_type& operator[](const Key_type& k) {
      auto found_it = find(k);
          if (found_it != end()) {
              ...
              insert({k,Value_type()});
              Value_type & answer = (*find(k)).second;
              return answer;
          }
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
     auto bleh = find(val.first);
      if (bleh != end()) {      
          return {bleh,false};
      }
      else {
        bst.insert(val);
        auto bleh2 = find(val.first);
        return {bleh2,true};
      }
  }
